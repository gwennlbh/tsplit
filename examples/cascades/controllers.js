...
export async function computeCascades(
    ...
    return await Promise.all(
        ...
        const groupedByOption = groupBy(// Get a list of { option id, confidence } for every cascaded value,
        // the confidence coming from the value that triggers it
        options.filter(nonnull).flatMap((
            {
                cascade,
                confidence
            }
